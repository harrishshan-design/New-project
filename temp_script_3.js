...
document.body.classList.toggle("master",sessionRole==="master");
document.body.classList.toggle("agent",sessionRole==="agent");
document.body.classList.toggle("user",sessionRole==="user");
...
"follow-up-builder":{role:"agent",badge:"Agent Experience",title:"Automated Follow-Up Builder",purpose:"Automate persuasion and consistency without losing a personal touch.",vibe:"Structured, efficient, template-driven.",description:"A simple block-style builder for WhatsApp, reminders, and recommendation sequences.",cards:[["Flow Builder","Day 1 → WhatsApp, Day 3 → Recommendation"],["Templates","Reusable messages for property nudges"],["Sequence Logic","Map stages by timeline"],["Agent Efficiency","Less manual chasing, more conversion"]],bullets:["Block-style sequence builder","Day-based follow-up automation","Template examples like 'I found a better unit for you...'","Easy edits for campaign flow"]},
...
function getAssignableAgents(){return readLocalAgents().filter(item=>item.activeToday&&item.verified)}
...
function getAgentPerformanceSnapshot(agent){const leads=readLocalLeads().filter(item=>item.assignedAgentPhone===agent.phone),bookings=readLocalBookings().filter(item=>item.assignedAgentPhone===agent.phone),tenancies=readLocalTenancies().filter(item=>item.agentPhone===agent.phone),confirmedViewings=bookings.filter(item=>item.status==="confirmed").length,closedDeals=tenancies.filter(item=>item.rentCollected||item.agreementSigned).length,responseMinutes=getAgentAverageResponseMinutes(agent.phone),responseScore=Math.max(38,100-responseMinutes*4),conversionRate=Math.round(((confirmedViewings+closedDeals)/Math.max(leads.length+bookings.length,1))*100),revenueGenerated=tenancies.reduce((sum,item)=>sum+Number(item.monthlyRent||0),0),liveLoad=leads.length+bookings.filter(item=>item.status==="pending").length+tenancies.filter(item=>!item.rentCollected).length,aiScore=Math.max(48,Math.min(99,Math.round(conversionRate*.38+responseScore*.27+(agent.verified?12:0)+(agent.activeToday?10:0)+Math.min(revenueGenerated/1500,18))));return{leads:leads.length,bookings:bookings.length,confirmedViewings,closedDeals,responseMinutes,responseScore,conversionRate,revenueGenerated,liveLoad,aiScore}}
function rankAgentsForOpportunity(context={}){const area=context.area||"",agents=getAssignableAgents();return agents.map(agent=>{const performance=getAgentPerformanceSnapshot(agent),areaScore=getAreaExpertiseScore(agent,area),loadPenalty=Math.min(18,performance.liveLoad*2),matchScore=Math.max(52,Math.min(99,Math.round(areaScore*.34+performance.responseScore*.23+performance.conversionRate*.19+performance.aiScore*.24-loadPenalty))),reason=area?`${agent.name} ranks strongly for ${area} because of ${areaScore>=82?"clear area expertise":"usable area fit"}, ${performance.responseMinutes} min response speed, and ${performance.conversionRate}% conversion health.`:`${agent.name} ranks strongly on performance, ${performance.responseMinutes} min response speed, and ${performance.conversionRate}% conversion health.`;return{...agent,...performance,areaScore,matchScore,reason}}).sort((a,b)=>b.matchScore-a.matchScore||a.liveLoad-b.liveLoad||a.responseMinutes-b.responseMinutes)}
function getNextAssignedAgent(context={}){const ranked=rankAgentsForOpportunity(context);if(!ranked.length)return null;const topScore=ranked[0].matchScore;const contenders=ranked.filter(agent=>Math.abs(agent.matchScore-topScore)<=4);const lastIndex=readRotationIndex();const nextIndex=(lastIndex+1)%contenders.length;writeRotationIndex(nextIndex);return contenders[nextIndex]}
function getLeadIntelligence(item){const property=properties.find(p=>p.id===item.listingId),behavior=analyzeBehavioralProfile(item.userAccount||item.userName),message=String(item.userMessage||"").toLowerCase();let score=item.kind==="booking"||item.requestedDate?76:54;if(behavior){score+=Math.min(behavior.viewedCount*3,12);score+=Math.min(behavior.savedCount*4,12);if(/invest/i.test(behavior.intent)&&property?.yield>=4)score+=9;if(/own-stay|family/i.test(behavior.intent)&&property?.bedrooms>=3)score+=8;if(behavior.avgBudgetValue&&property?.price<=behavior.avgBudgetValue*1.08)score+=7}if(message.includes("roi")||message.includes("yield")||message.includes("investment"))score+=8;if(message.includes("urgent")||message.includes("today")||message.includes("now"))score+=6;if(message.includes("expensive")||message.includes("budget"))score+=4;if(item.kind==="booking")score+=10;score=Math.max(32,Math.min(98,score));const tone=score>=80?"hot":score>=62?"warm":"cold",toneLabel=tone==="hot"?"HOT buyer":tone==="warm"?"Warm buyer":"Cold buyer",toneClass=tone==="hot"?"confirmed":tone==="warm"?"pending":"declined",intentLabel=behavior?.intent||(/investment|yield|roi/.test(message)?"Investor":"Decision-stage buyer"),preference=behavior?.preferenceLine||`${property?.type||"property"} opportunities`,suggestedAngle=/expensive|budget/.test(message)||behavior?.avgBudgetValue&&property?.price>behavior.avgBudgetValue?`This buyer is price sensitive. Lead with a negotiation angle around ${rmFull(getDecisionPack(property).negotiation)} and explain why the area still protects downside.`:/invest/i.test(intentLabel)?`This buyer thinks like an investor. Lead with ${property?.yield||0}% yield, ${property?.growth||0}% growth, and the undervalue gap.`:`This buyer needs confidence more than raw data. Lead with lifestyle fit, verification, and the easiest next viewing slot.`,suggestedReply=`Hi ${item.userName}, I reviewed your activity around ${property?.title||"this listing"}. The strongest angle for you is ${/invest/i.test(intentLabel)?"yield plus negotiation room":"fit plus lower-regret downside"}. I can show you the clearest next-step option today if you want.`,objectionCounter=/expensive|budget/.test(message)||behavior?.avgBudgetValue&&property?.price>behavior.avgBudgetValue?`If they say it is too expensive, counter with fair value ${rmFull(getFairValue(property))}, negotiation zone ${rmFull(getDecisionPack(property).negotiation)}, and why ${property?.area||"the area"} still supports demand.`:`If they hesitate, counter with why ${property?.area||"the area"} fits ${intentLabel.toLowerCase()} demand and what risk is already controlled.`,reasonLine=behavior?`${behavior.strategyLine} Current preference: ${behavior.preferenceLine}.`:`The system sees fresh interest around ${property?.area||"this area"} but needs one decisive follow-up to move this lead.`,followUps=[{label:"Day 1",text:`Hi ${item.userName}, still looking around ${property?.area||"this area"}? I have one angle that fits your brief better than the public card.`},{label:"Day 3",text:`Quick update: pricing around ${property?.area||"this area"} moved again and I found a stronger match for your profile.`},{label:"Day 7",text:`A new unit now matches the taste you showed around ${preference}. Want the shortlist before it gets noisy?`}];return{score,tone,toneLabel,toneClass,intentLabel,preference,suggestedAngle,suggestedReply,objectionCounter,reasonLine,followUps}}
...
savedIds=added?[...savedIds,id]:savedIds.filter(x=>x!==id);renderProperties();renderCompare();popActiveControl();tapFeedback(added?"Saved to your vault":"Removed from saved homes",properties.find(item=>item.id===id)?.title||"Property updated",added?"success":"alert")}
...
function showInvestorAnalyzer(){const panel=$("investorAnalyzerPanel");if(!panel)return;panel.style.display="block";panel.style.animation="slideDown 0.4s ease";const sel=$("invPropertySelect");if(sel){sel.innerHTML=properties.map(p=>`<option value="${p.id}">${p.title} — ${p.location} (${money(p.price)})</option>`).join("");runInvestorAnalysis();}scrollToSection("investorAnalyzerPanel");}
...
const yieldBar=Math.min(100,Math.round(yieldPct/8*100));$("invYieldValue").textContent=yieldPct+"%";$("invYieldNote").textContent=yieldPct>=5?"✅ Above market average — strong cash flow asset":yieldPct>=4?"⚡ At market average — decent income play":"⚠️ Below average — appreciate-heavy bet";$("invYieldBar").style.width=yieldBar+"%";// Predict card
...
const score=Math.min(100,Math.round((growthPct/20*60)+(yieldPct/8*40)));const grade=score>=80?"A+ Excellent":score>=65?"B+ Strong":score>=50?"C+ Moderate":"D Below Average";$("invGrowthValue").textContent=score+" / 100";$("invGrowthNote").textContent=grade+" — "+p.vibe;$("invGrowthBar").style.width=score+"%";// Verdict
const verdict=score>=80?"🔥 Hot pick for investors — high yield and strong growth make this a rare combo.":score>=65?"✅ Solid investment — above-average returns and reliable area fundamentals.":score>=50?"⚡ Moderate option — worth holding if you have a 7+ year horizon.":"⚠️ Caution — consider better-yielding alternatives in the same budget.";$("invVerdict").innerHTML=`<div class="inv-verdict-inner"><i class="fas fa-brain"></i> <strong>AI Verdict:</strong> ${verdict} <br><span style="color:var(--muted);font-size:0.9rem;">Based on ${p.area} area signals, current yield of ${yieldPct}%, and projected ${growthPct}% annual capital growth.</span></div>`;$("invResultsWrap").style.display="block";// Pre-fill calculator
...
function saveListingEdits(){if(sessionRole!=="master"||activeModalId==null)return;const item=properties.find(p=>p.id===activeModalId);if(!item)return;item.title=$("editTitle").value.trim()||item.title;item.location=$("editLocation").value.trim()||item.location;item.area=$("editArea").value.trim()||item.area;item.price=Number($("editPrice").value)||item.price;item.type=$("editType").value;item.bedrooms=Number($("editBedrooms").value)||item.bedrooms;item.bathrooms=Number($("editBathrooms").value)||item.bathrooms;item.sqft=Number($("editSqft").value)||item.sqft;item.psf=Number($("editPsf").value)||item.psf;item.yield=Number($("editYield").value)||item.yield;item.growth=Number($("editGrowth").value)||item.growth;item.image=$("editImage").value.trim()||item.image;item.mapLink=$("editMap").value.trim()||item.mapLink;item.vibe=$("editVibe").value.trim()||item.vibe;item.commute=$("editCommute").value.trim()||item.commute;item.fit=$("editFit").value.trim()||item.fit;item.verifiedType=$("editVerified").value;saveMasterListings();renderProperties();renderCompare();fillModal(item);$("modalStatus").textContent=`Saved changes for ${item.title}.`;$("adminStatus").textContent=`Updated ${item.title} from the detail editor.`;loadMasterExecutive();}
...
function toggleMasterSection(id,forceOpen){const section=$(id);if(!section||sessionRole!=="master")return;const shouldOpen=forceOpen===true?true:section.classList.contains("master-collapsed");section.classList.toggle("master-collapsed",!shouldOpen);updateMasterSectionToggle(id)}
...
function loadMasterExecutive(){if(sessionRole!=="master")return;const agents=readLocalAgents(),leads=readLocalLeads().slice().sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)),bookings=readLocalBookings().slice().sort((a,b)=>new Date(b.updatedAt||b.createdAt)-new Date(a.updatedAt||a.createdAt)),tenancies=readLocalTenancies().slice().sort((a,b)=>new Date(b.updatedAt||b.createdAt)-new Date(a.updatedAt||a.createdAt)),chats=readLocalChats().slice().sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)),signalLog=readLocalAreaInterest().filter(item=>item.day===todayKey()),areas=getAreaSignalSummary(),activeAgentCount=agents.filter(item=>item.activeToday).length,verifiedAgentCount=agents.filter(item=>item.verified).length,confirmedViewings=bookings.filter(item=>item.status==="confirmed").length,pendingViewings=bookings.filter(item=>item.status==="pending").length,activeRentals=tenancies.length,rentCollected=tenancies.filter(item=>item.rentCollected).length,liveSignals=signalLog.reduce((sum,item)=>sum+Number(item.count||0),0),activeUsers=new Set([...leads.map(item=>item.userPhone||item.userName),...bookings.map(item=>item.userPhone||item.userName),...chats.map(item=>`${item.role}:${item.username}`)].filter(Boolean)).size,topArea=areas[0],totalLeadPressure=leads.length+bookings.length,callbacks=signalLog.filter(item=>item.source==="callback").reduce((sum,item)=>sum+Number(item.count||0),0),viewMore=signalLog.filter(item=>item.source==="view_more").reduce((sum,item)=>sum+Number(item.count||0),0),tourRequests=signalLog.filter(item=>item.source==="tour_request").reduce((sum,item)=>sum+Number(item.count||0),0),matchSignals=signalLog.filter(item=>item.source==="match").reduce((sum,item)=>sum+Number(item.count||0),0),conversion=totalLeadPressure?Math.round(((confirmedViewings+rentCollected)/totalLeadPressure)*100):0;if($("masterPulseHeadline"))$("masterPulseHeadline").textContent=totalLeadPressure?`${totalLeadPressure} live buyer action${totalLeadPressure===1?"":"s"} need oversight`:"No live buyer pressure yet";if($("masterPulseText"))$("masterPulseText").textContent=totalLeadPressure?`${leads.length} callback lead${leads.length===1?"":"s"}, ${pendingViewings} pending tour${pendingViewings===1?"":"s"}, and ${confirmedViewings} confirmed viewing${confirmedViewings===1?"":"s"} are flowing through the platform.`:"The command center will start filling once users browse, ask for contact, or book tours.";if($("masterAreaHeadline"))$("masterAreaHeadline").textContent=topArea?`${topArea.area} is leading buyer attention`:"No dominant area yet";if($("masterAreaText"))$("masterAreaText").textContent=topArea?`${topArea.count} anonymous signal${topArea.count===1?"":"s"} today. Strongest sources: ${Object.entries(topArea.sources).sort((a,b)=>b[1]-a[1]).slice(0,2).map(entry=>`${entry[0].replace(/_/g," ")} ${entry[1]}`).join(" • ")}.`:"Buyer behavior heat will highlight which areas deserve extra agent and listing focus.";if($("masterKpiAgents"))$("masterKpiAgents").textContent=activeAgentCount;if($("masterKpiAgentsNote"))$("masterKpiAgentsNote").textContent=`${verifiedAgentCount} verified / ${agents.length} total rostered`;if($("masterKpiLeads"))$("masterKpiLeads").textContent=leads.length;if($("masterKpiLeadsNote"))$("masterKpiLeadsNote").textContent=`${activeUsers} active user touchpoint${activeUsers===1?"":"s"} tracked`;if($("masterKpiViewings"))$("masterKpiViewings").textContent=bookings.length;if($("masterKpiViewingsNote"))$("masterKpiViewingsNote").textContent=`${pendingViewings} pending • ${confirmedViewings} confirmed`;if($("masterKpiRentals"))$("masterKpiRentals").textContent=activeRentals;if($("masterKpiRentalsNote"))$("masterKpiRentalsNote").textContent=`${rentCollected} rent collected • ${Math.max(activeRentals-rentCollected,0)} still open`;if($("masterKpiChats"))$("masterKpiChats").textContent=chats.length;if($("masterKpiChatsNote"))$("masterKpiChatsNote").textContent=`${chats.filter(item=>item.role==="agent").length} agent chat${chats.filter(item=>item.role==="agent").length===1?"":"s"} • ${chats.filter(item=>item.role==="user").length} user chat${chats.filter(item=>item.role==="user").length===1?"":"s"}`;if($("masterKpiSignals"))$("masterKpiSignals").textContent=liveSignals;if($("masterKpiSignalsNote"))$("masterKpiSignalsNote").textContent=`${areas.length} active area${areas.length===1?"":"s"} showing intent`;if($("masterOverviewLeadHeadline"))$("masterOverviewLeadHeadline").textContent=totalLeadPressure?`${conversion}% blended conversion health across live demand`:"Lead conversion health will appear here";if($("masterOverviewLeadText"))$("masterOverviewLeadText").textContent=totalLeadPressure?`The platform is carrying ${leads.length} callback lead${leads.length===1?"":"s"} and ${bookings.length} viewing booking${bookings.length===1?"":"s"}. Focus first on ${pendingViewings} booking${pendingViewings===1?"":"s"} still waiting on agent action.`:"No leads or tours are moving through the funnel yet.";if($("masterOverviewAgentHeadline"))$("masterOverviewAgentHeadline").textContent=agents.length?`${activeAgentCount} active agent${activeAgentCount===1?"":"s"} are carrying today's load`:"No agent roster yet";if($("masterOverviewAgentText"))$("masterOverviewAgentText").textContent=agents.length?`${verifiedAgentCount} verified login${verifiedAgentCount===1?"":"s"} are ready. ${agents.filter(item=>!item.verified).length} still need master approval and ${agents.filter(item=>!item.activeToday).length} are inactive today.`:"Add agents and verify them so the lead rotation and oversight views can activate.";if($("masterOverviewRentalHeadline"))$("masterOverviewRentalHeadline").textContent=activeRentals?`${activeRentals} rental deal${activeRentals===1?"":"s"} in the audit trail`:"No rental deals yet";if($("masterOverviewRentalText"))$("masterOverviewRentalText").textContent=activeRentals?`${tenancies.filter(item=>item.keysShared).length} key handover${tenancies.filter(item=>item.keysShared).length===1?"":"s"}, ${tenancies.filter(item=>item.agreementSigned).length} agreement${tenancies.filter(item=>item.agreementSigned).length===1?"":"s"} signed, and ${rentCollected} rent collection${rentCollected===1?"":"s"} completed so far.`:"The rental audit will summarize keys, agreements, and payments once agents start moving tenant deals forward.";if($("masterOverviewBehaviorHeadline"))$("masterOverviewBehaviorHeadline").textContent=liveSignals?`${liveSignals} anonymous behavior signal${liveSignals===1?"":"s"} today`:"No behavior data yet";if($("masterOverviewBehaviorText"))$("masterOverviewBehaviorText").textContent=liveSignals?`${viewMore} view-more opens, ${callbacks} callback request${callbacks===1?"":"s"}, ${tourRequests} tour request${tourRequests===1?"":"s"}, and ${matchSignals} match-inspection signal${matchSignals===1?"":"s"} are shaping the heat map.`:"User intent will show up here after people start opening listings, checking matches, and asking for agent contact.";const signalCards=[{label:"Callbacks",value:callbacks,note:"Users who asked to speak with an agent."},{label:"View More",value:viewMore,note:"Deep property interest from detail opens."},{label:"Tour Requests",value:tourRequests,note:"Higher-intent buyers asking for a slot."}];if($("masterSignalStats"))$("masterSignalStats").innerHTML=signalCards.map(item=>`<article class="master-mini-card"><span>${item.label}</span><strong>${item.value}</strong><p>${item.note}</p></article>`).join("");if($("masterAgentDeskMeta"))$("masterAgentDeskMeta").textContent=agents.length?`${agents.length} total agents, ${activeAgentCount} active today`:"No agents added yet";if($("masterAgentDesk"))$("masterAgentDesk").innerHTML=agents.length?agents.map(agent=>{const leadCount=leads.filter(item=>item.assignedAgentPhone===agent.phone).length,bookingCount=bookings.filter(item=>item.assignedAgentPhone===agent.phone).length,tenantCount=tenancies.filter(item=>item.agentPhone===agent.phone).length;return`<div class="compare-item"><div><strong>${agent.name}</strong><p>${agent.company||"Independent agent"}${agent.areaFocus?` • ${agent.areaFocus}`:""}</p><div class="mini">${agent.phone}</div><p class="sub" style="margin-top:6px">Leads: ${leadCount} • Tours: ${bookingCount} • Rentals: ${tenantCount}</p><p class="sub" style="margin-top:6px">${agent.verified?"Verified and login-ready":"Awaiting verification"}${agent.activeToday?" • Active today":" • Inactive today"}</p></div><div class="actions"><span class="booking-status ${agent.verified?"confirmed":"declined"}">${agent.verified?"Verified":"Pending Verify"}</span><span class="booking-status ${agent.activeToday?"pending":"declined"}">${agent.activeToday?"Active Today":"Inactive"}</span></div></div>`}).join(""):'<p class="empty">No agents added yet. Once you add agents, this desk will show workload, verification, and rotation readiness.</p>';const leadFeed=[...leads.map(item=>({kind:"Callback Lead",status:"New lead",tone:"confirmed",time:item.createdAt,user:item.userName,phone:item.userPhone,title:item.listingTitle,agent:item.assignedAgentName,note:item.userMessage||"No message provided."})),...bookings.map(item=>({kind:"Viewing Booking",status:item.status==="confirmed"?"Accepted":item.status==="declined"?"Declined":"Pending action",tone:bookingStatusMeta(item.status).className,time:item.updatedAt||item.createdAt,user:item.userName,phone:item.userPhone,title:item.listingTitle,agent:item.assignedAgentName,note:`Requested ${formatTourSlot(item.requestedDate,item.requestedTime)}${item.respondedBy?` • Responded by ${item.respondedBy}`:""}`}))].sort((a,b)=>new Date(b.time)-new Date(a.time));if($("masterLeadDeskMeta"))$("masterLeadDeskMeta").textContent=leadFeed.length?`${leadFeed.length} live lead and viewing record${leadFeed.length===1?"":"s"}`:"No lead capture yet";if($("masterLeadDesk"))$("masterLeadDesk").innerHTML=leadFeed.length?leadFeed.slice(0,12).map(item=>`<div class="compare-item"><div><strong>${item.user}</strong><p>${item.title}</p><div class="mini">${item.phone}</div><p class="sub" style="margin-top:6px">${item.note}</p><p class="sub" style="margin-top:6px">${item.kind} • Assigned to ${item.agent}</p><p class="sub" style="margin-top:6px">Updated ${formatLeadTime(item.time)}</p></div><div class="actions"><span class="booking-status ${item.tone}">${item.status}</span></div></div>`).join(""):'<p class="empty">No callback leads or viewing bookings tracked yet.</p>';const behaviorStats=[{label:"Active Areas",value:areas.length||0,note:"Areas with live anonymous attention today."},{label:"Top Area",value:topArea?topArea.area:"None",note:"Strongest concentration of buyer activity."},{label:"Tracked Users",value:activeUsers||0,note:"Unique user touchpoints from leads, tours, and chats."}];if($("masterBehaviorStats"))$("masterBehaviorStats").innerHTML=behaviorStats.map(item=>`<article class="master-mini-card"><span>${item.label}</span><strong>${item.value}</strong><p>${item.note}</p></article>`).join("");if($("masterBehaviorDeskMeta"))$("masterBehaviorDeskMeta").textContent=areas.length?`${areas.length} area cluster${areas.length===1?"":"s"} updated from live browsing and request behavior`:"No browsing concentration yet";if($("masterBehaviorDesk"))$("masterBehaviorDesk").innerHTML=areas.length?areas.slice(0,8).map(item=>{const sourceLine=Object.entries(item.sources).sort((a,b)=>b[1]-a[1]).map(entry=>`${entry[0].replace(/_/g," ")} ${entry[1]}`).join(" • ");return`<div class="compare-item"><div><strong>${item.area}</strong><p>This area is attracting more anonymous user attention than most others today.</p><p class="sub" style="margin-top:6px">${sourceLine}</p></div><div class="actions"><span class="booking-status confirmed">${item.count} signals</span></div></div>`}).join(""):'<p class="empty">Once buyers start opening listings, using match logic, requesting callbacks, or booking tours, behavior heat will appear here.</p>';if($("masterRentalDeskMeta"))$("masterRentalDeskMeta").textContent=tenancies.length?`${tenancies.length} rental deal${tenancies.length===1?"":"s"} in audit`:"No tenancies in the audit trail yet";if($("masterRentalDesk"))$("masterRentalDesk").innerHTML=tenancies.length?tenancies.slice(0,10).map(item=>{const meta=tenancyStageMeta(item);return`<div class="compare-item"><div><strong>${item.tenantName}</strong><p>${item.listingTitle}</p><div class="mini">${item.tenantPhone}</div><p class="sub" style="margin-top:6px">Handling agent: ${item.agentName} • Owner: ${item.ownerName}</p><p class="sub" style="margin-top:6px">Keys: ${item.keysShared?`Shared by ${item.keysMarkedBy||item.agentName}`:"Pending"} • Agreement: ${item.agreementSigned?`Signed by ${item.agreementMarkedBy||item.agentName}`:"Pending"} • Rent: ${item.rentCollected?`Collected by ${item.rentMarkedBy||item.agentName}`:"Pending"}</p><p class="sub" style="margin-top:6px">Payment: ${item.paymentStatus==="submitted"?`${rmFull(item.paymentAmount)} via ${item.paymentMethod||"manual"}`:"Not submitted yet"}</p></div><div class="actions"><span class="booking-status ${meta.className}">${meta.label}</span></div></div>`}).join(""):'<p class="empty">No selected tenant deals yet. Once an agent chooses a tenant, the rental database will populate here.</p>';if($("masterChatDeskMeta"))$("masterChatDeskMeta").textContent=chats.length?`${chats.length} tracked conversation${chats.length===1?"":"s"} across user and agent accounts`:"No chat activity yet";if($("masterChatDesk"))$("masterChatDesk").innerHTML=chats.length?chats.slice(0,10).map(item=>`<div class="compare-item"><div><strong>${item.username}</strong><p>${item.message}</p><p class="sub" style="margin-top:6px">${item.reply}</p><div class="mini">${item.role} • ${formatLeadTime(item.createdAt)}${item.feedback?` • ${item.feedback.replace("_"," ")}`:""}</div></div><div class="actions"><span class="booking-status ${item.role==="agent"?"pending":"confirmed"}">${item.role==="agent"?"Agent Chat":"User Chat"}</span></div></div>`).join(""):'<p class="empty">Chat tracking will appear after users or agents talk to the property chatbot.</p>'}
function renderPowerCompare(){const list=powerFilteredIds.length?properties.filter(item=>powerFilteredIds.includes(item.id)).sort((a,b)=>powerFilteredIds.indexOf(a.id)-powerFilteredIds.indexOf(b.id)):[];const container=$("powerCompareList"),count=$("powerCompareCount"),status=$("powerModeStatus"),meta=$("masterPowerMeta");if(!container||!count||!status||!meta)return;if(!list.length){count.textContent="0 shortlisted for compare";meta.textContent="Advanced investor and export workflow";status.textContent="Use yield, growth, and risk to build a serious-investor comparison set.";container.innerHTML='<p class="empty">No investor shortlist yet. Set a minimum yield, growth target, or risk level, then run bulk compare.</p>';return}const validSelection=masterBulkCompareIds.filter(id=>powerFilteredIds.includes(id));masterBulkCompareIds=validSelection.length?validSelection:list.slice(0,Math.min(4,list.length)).map(item=>item.id);count.textContent=`${masterBulkCompareIds.length} selected for compare`;meta.textContent=`${list.length} filtered opportunities ready for serious-investor review`;container.innerHTML=list.map(property=>{const decision=getDecisionPack(property),checked=masterBulkCompareIds.includes(property.id),undervalue=Math.max(Math.round(getUndervalueRatio(property)*100),0);return`<label class="bulk-compare-item"><input type="checkbox" ${checked?"checked":""} onchange="togglePowerCompare(${property.id},this.checked)"><div><strong>${property.title}</strong><p>${property.location}</p><div class="mini">${money(property.price)} • ${property.yield}% yield • ${property.growth}% growth • Risk ${decision.risk}</div><p class="sub" style="margin-top:8px">${decision.bullets[0]}</p></div><div class="agent-metrics"><b>${rmFull(decision.negotiation)}</b><b>${undervalue}% below fair</b></div></label>`}).join("")}
...
function regenerateAgentContent(){if(sessionRole!=="agent")return;const store=readLocalAgentContent();delete store[`${sessionAgentPhone||"all"}_${todayKey()}`];writeLocalAgentContent(store);renderContentGenerator();$("contentGeneratorStatus").textContent="Generated a fresh set of hook ideas for today."}
...
async function loadAgentInbox(){if(sessionRole!=="agent")return;const status=$("agentInboxStatus");const leads=readLocalLeads().filter(item=>item.assignedAgentPhone===sessionAgentPhone).map(item=>({...item,kind:"lead"}));const bookings=readLocalBookings().filter(item=>item.assignedAgentPhone===sessionAgentPhone).map(item=>({...item,kind:"booking"}));agentInbox=[...bookings,...leads].sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));renderAgentInbox();status.textContent=agentInbox.length?`Inbox updated for ${sessionName}.`:"No leads or tour requests assigned yet.";loadAutomationConsole();loadAgentDailyTools()}
...
function loadAgentDailyTools(){if(sessionRole!=="agent")return;renderDailyActionList();renderGeoLeadMap();renderContentGenerator()}
...
function loadChatLogs(){if(sessionRole!=="master")return;const status=$("chatLogStatus");chatLogs=readLocalChats();renderChatLogs();status.textContent=chatLogs.length?`Showing chatbot activity for ${chatLogs.length} message${chatLogs.length===1?"":"s"}.`:"No chatbot activity yet.";loadMasterExecutive()}
...
function loadRentalManagement(){if(sessionRole!=="master")return;const status=$("rentalManagementStatus");const items=readLocalTenancies();renderRentalManagement();status.textContent=items.length?`Managing ${items.length} rental handoff${items.length===1?"":"s"} across owners, tenants, and agents.`:"No rental management records yet.";loadMasterExecutive()}
...
function respondToBooking(id,nextStatus){if(sessionRole!=="agent")return;const bookings=readLocalBookings();const current=bookings.find(item=>item.id===id&&item.assignedAgentPhone===sessionAgentPhone);if(!current)return;const updatedRecord={...current,status:nextStatus,updatedAt:new Date().toISOString(),respondedBy:sessionName};writeLocalBookings(bookings.map(item=>item.id===id?updatedRecord:item));createNotification({userName:current.userName,title:nextStatus==="confirmed"?"Your viewing tour is booked":"Please choose another viewing date",message:nextStatus==="confirmed"?`${sessionName} accepted your viewing for ${current.listingTitle} on ${formatTourSlot(current.requestedDate,current.requestedTime)}.`:`${sessionName} declined your requested slot for ${current.listingTitle}. Please pick another date and time from your shortlist.`});loadAgentInbox();loadAutomationConsole()}
async 
...
/* // Decision-engine overrides for buyer-facing surfaces.
async function runOnboarding(){
    if(sessionRole!=="user")return;
    const budget=$("onboardingBudget").value,location=$("onboardingLocation").value,goal=$("onboardingGoal").value,status=$("onboardingStatus");
    $("priceRange").value=budget;
    $("location").value=location;
    $("buyerGoal").value=goal;
    if(typeof updateBuyerMemory==="function")updateBuyerMemory("focused",null,{budget,location,goal});
    status.innerHTML='<i class="fas fa-circle-notch fa-spin"></i> Refining your strategy with live market and behavior data...';
    try{
        const payload={budget,location,goal,properties};
        const res=await fetch('http://localhost:3000/api/agents/rank',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
        const rankedArray=await res.json();
        if(rankedArray.error||rankedArray.fallback){
            renderProperties();
            renderAIOrganizerView();
            const count=filtered().length;
            status.textContent=`We refined your search locally and found ${count} higher-fit matches for this strategy.`;
            scrollToSection("discover");
            return;
        }
        const curatedProps=rankedArray.map(rank=>{const property=properties.find(x=>x.id===rank.id);return property?{...property,aiExplanation:rank.explanation}:null}).filter(Boolean);
        const behavior=analyzeBehavioralProfile();
        document.getElementById("aiCuratedHeader").innerHTML=`<span style="font-weight:700; color:var(--brand); text-transform:uppercase; font-size:0.8rem; letter-spacing:0.04em;">Strategy Refined</span><h3 style="margin:8px 0; font-family:'Outfit', sans-serif; font-size: 1.4rem;">Top 3 Curated Properties</h3><p style="margin:0; font-weight:500; font-size:1.1rem; color: var(--ink); line-height: 1.5;">${behavior?behavior.strategyLine:"We refined your shortlist using budget, location, and goal signals."}</p>`;
        document.getElementById("aiCuratedGrid").innerHTML=curatedProps.map((p,index)=>{const media=getPropertyMedia(p),decision=getDecisionPack(p);return `<article class="card glass" style="border: 2px solid var(--brand); transform: scale(1.02); z-index:2; box-shadow: 0 12px 40px rgba(187,77,45,0.15);">
                <div class="media" style="height:220px;"><img src="${media.images[0]}" loading="lazy" decoding="async">
                <div class="tint"></div>
                <div class="topline">
                    <div class="stack">${p.badge?`<span class="pill ${p.badge}">${p.badge}</span>`:``} <span class="pill">${p.type}</span></div>
                    <div class="stack"><span class="score"><i class="fas fa-bolt"></i> AI Ranked #${index+1}</span></div>
                </div></div>
                <div class="body">
                    <div class="price-row">
                        <div>${priceMarkup || `<div class="price">${money(p.price)}</div>`}<div class="title" style="font-family:'Outfit', sans-serif; font-size:1.2rem; font-weight:700;">${p.title}</div></div>
                    ...
                    <div style="margin: 16px 0; padding: 14px; background: rgba(187,77,45,0.06); border-radius: 12px; border-left: 3px solid var(--brand);">
                        <strong style="color:var(--brand-dark); font-size:0.8rem; letter-spacing:0.02em; text-transform:uppercase;"><i class="fas fa-microchip"></i> Decision Brain</strong>
                        <p style="margin: 6px 0 0 0; font-size: 0.95rem; line-height:1.45; color:#332;">${p.aiExplanation||decision.personalSummary}</p>
                    </div>
                    <div class="actions">
                        <span class="mini">ROI: ${decision.roi}%</span>
                        <span class="mini">Risk: ${decision.risk}</span>
                        <span class="mini">Offer: ${rmFull(decision.negotiation)}</span>
                    </div>
                    <div class="actions" style="margin-top:12px;">
                        <button class="btn" style="width:100%" onclick="openModal(${p.id})">Analyze Match</button>
                    </div>
        ...
        status.innerHTML='<i class="fas fa-check-circle" style="color:#059669"></i> Your shortlist was refined using decision-engine logic.';
        renderProperties();
        renderAIOrganizerView();
        const target=document.getElementById("aiCuratedGrid");
        if(target)target.scrollIntoView({behavior:"smooth",block:"center"});
    }catch(err){
        console.error(err);
        status.textContent="AI core is offline, but we still refined your shortlist locally.";
        renderProperties();
        renderAIOrganizerView();
    }
}
...
<i class="fas fa-microchip" style="color:var(--brand);"></i> <b>AI Verdict:</b> Generated ${Math.floor(Math.random()*90)+10}% confidence match for your profile.
            ...
            <div class="actions">
                ...
                <span class="mini">🔥 ${liveCount} viewing now</span>
                <span class="mini">Risk: ${decision.risk}</span>
...
const property=properties.find(item=>item.id===id),added=!savedIds.includes(id);savedIds=added?[...savedIds,id]:savedIds.filter(x=>x!==id);if(property&&added&&sessionRole==="user"&&typeof updateBuyerMemory==="function")updateBuyerMemory("saved",property);renderProperties();renderCompare();renderAIOrganizerView();popActiveControl();tapFeedback(added?"Saved to your vault":"Removed from saved homes",property?.title||"Property updated",added?"success":"alert")}
...
function renderDopamineAgentDashboard(){if(sessionRole!=="agent")return;const myAgent=readLocalAgents().find(item=>item.phone===sessionAgentPhone)||{name:sessionName,phone:sessionAgentPhone,areaFocus:"Klang Valley",verified:true,activeToday:true},leads=readLocalLeads().filter(item=>item.assignedAgentPhone===sessionAgentPhone).map(item=>({...item,kind:"lead"})),bookings=readLocalBookings().filter(item=>item.assignedAgentPhone===sessionAgentPhone).map(item=>({...item,kind:"booking"})),signals=[...bookings,...leads].sort((a,b)=>new Date(b.updatedAt||b.createdAt)-new Date(a.updatedAt||a.createdAt)),performance=getAgentPerformanceSnapshot(myAgent),uniqueBuyers=new Set(signals.map(item=>item.userPhone||item.userName).filter(Boolean)).size,topSignal=signals[0],topIntel=topSignal?getLeadIntelligence(topSignal):null;if($("dopamineActiveLeads"))$("dopamineActiveLeads").textContent=signals.length;if($("dopamineActiveViewings"))$("dopamineActiveViewings").textContent=performance.confirmedViewings||bookings.length;if($("dopamineClosingRate"))$("dopamineClosingRate").textContent=`${performance.conversionRate}%`;if($("dopamineMarketBuyers"))$("dopamineMarketBuyers").textContent=uniqueBuyers||signals.length;if($("dopamineMarketLeadsBadge"))$("dopamineMarketLeadsBadge").innerHTML=`<i class="fas fa-bolt"></i> ${signals.length} live signals`;if($("dopaminePipeNew"))$("dopaminePipeNew").textContent=`New Leads (${leads.length})`;if($("dopaminePipeView"))$("dopaminePipeView").textContent=`Viewing (${bookings.filter(item=>item.status==="pending").length})`;if($("dopamineNextActionTitle"))$("dopamineNextActionTitle").textContent=topSignal?`Prioritize ${topSignal.userName} — ${topIntel.score}% ${topIntel.toneLabel}`:"No live buyer action yet";if($("dopamineNextActionReason"))$("dopamineNextActionReason").textContent=topIntel?topIntel.reasonLine:"Fresh leads, tours, and callbacks will create the next action automatically.";const signalWrap=$("liveBuyerSignalsList");if(signalWrap)signalWrap.innerHTML=signals.length?signals.slice(0,6).map(signal=>{const intel=getLeadIntelligence(signal),property=properties.find(p=>p.id===signal.listingId),phone=normalizePhone(signal.userPhone||""),replyText=intel.suggestedReply;return `<article class="card glass" style="padding:24px; border-left:6px solid ${intel.tone==="hot"?"#f97316":intel.tone==="warm"?"#ca8a04":"#94a3b8"}; border-radius:16px;">
                <div style="display:flex; justify-content:space-between; gap:14px; margin-bottom:12px;">
                    <div>
                        ...
                        <p class="mini">${property?.area||"Klang Valley"} • ${intel.intentLabel}</p>
                    </div>
                    <div style="text-align:right;">
...
async function contactAgent(){if(activeModalId==null)return;const property=properties.find(p=>p.id===activeModalId);if(!property)return;const name=$("leadName").value.trim()||sessionName||"Stealth Lead",phone=normalizePhone($("leadPhone").value),message=$("leadMessage").value.trim()||"Unlocked Data Pack",btn=$("contactAgentBtn");if(!phone){alert("Please enter your WhatsApp number to receive the data pack.");return}trackAreaInterest(property.area,"callback");const agent=getNextAssignedAgent({area:property.area,listingId:property.id,kind:"lead"});if(!agent){btn.textContent="No active agent right now";return}const behavior=analyzeBehavioralProfile(),leadEntry={id:Date.now(),userAccount:sessionUserAccount,userName:name,userPhone:phone,userMessage:message,leadName:name,leadPhone:phone,leadMessage:message,listingId:property.id,listingTitle:property.title,listingPrice:property.price,listingArea:property.area,behaviorIntent:behavior?.intent||"",behaviorPreference:behavior?.preferenceLine||"",assignedAgentPhone:agent.phone,assignedAgentName:agent.name,createdAt:new Date().toISOString()};btn.disabled=true;btn.textContent="Connecting to Agent...";try{writeLocalLeads([leadEntry,...readLocalLeads()]);await fetch('http://localhost:3000/api/leads',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(leadEntry)}).catch(()=>{});btn.textContent="Data Pack Sent! WhatsApp incoming.";btn.style.background="linear-gradient(135deg, #059669, #047857)";setTimeout(()=>{closeModal();loadAutomationConsole();renderAIOrganizerView();tapFeedback("Lead captured",`${agent.name} now has a scored lead in the agent inbox.`,"success")},1500)}catch(err){console.error(err);btn.disabled=false;btn.textContent="Error. Try Again."}}
*/
