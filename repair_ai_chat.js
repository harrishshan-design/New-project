...
// Fix the unclosed model-viewer script!
const badScript = '<script type="module" src="https://ajax.googleapis.com/ajax/libs/model-viewer/4.2.0/model-viewer.min.js">\\n\\n// AI CHAT LOGIC';
const goodScript = '<script type="module" src="https://ajax.googleapis.com/ajax/libs/model-viewer/4.2.0/model-viewer.min.js"></script>\\n<script>\\n// AI CHAT LOGIC';

if (content.includes(badScript)) {
    content = content.replace(badScript, goodScript);
    // Also we need to close the script block before the <style> starts
    const beforeStyle = '\\n<style>\\n:root{';
    const afterScriptClosed = '\\n</script>\\n<style>\\n:root{';
    if(content.includes(beforeStyle)) {
        content = content.replace(beforeStyle, afterScriptClosed);
    }
}
...
if (!content.includes('<!-- 🤖 AI CHAT SYSTEM -->')) {
    content = content.replace('</body>', htmlBlock);
}

// Ensure the CSS is present inside the final </style>
const cssBlock = `
...
if (!content.includes('.sticky-ai-toggle {')) {
    content = content.replace('</style>', cssBlock);
}

// Make sure that submitUserChat saves to CRM correctly. We rewrite it if needed, but it seems perfectly intact from earlier.
fs.writeFileSync('dashboard.html', content, 'utf8');
console.log('Restoration completed! Script tags fixed, and Ask Property AI logic properly restored.');
