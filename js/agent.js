...
function triggerSpeedFeedback(btn) {
    if(btn) { btn.innerHTML = '<i class="fas fa-check"></i> Action Recorded'; setTimeout(()=>btn.innerHTML='≡ƒÆ¼ Message Now', 3000); }
...
function renderDopamineAgentDashboard() {
    ...
    if(topLead) {
        if($("dopamineNextActionTitle")) $("dopamineNextActionTitle").textContent = `Contact ${topLead.userName} ΓÇö ${totalSignals > 0 ? 82 : 76}% closing probability`;
    ...
    if(document.getElementById("liveBuyerSignalsList")) {
        document.getElementById("liveBuyerSignalsList").innerHTML = allSignals.length ? allSignals.map((signal, idx) => {
            ...
            const badge = isHot ? '<span style="background:#fff7ed; color:#c2410c; padding:4px 10px; border-radius:6px; font-size:0.75rem; font-weight:800; letter-spacing:0.02em;">≡ƒöÑ HOT BUYER</span>' 
                                : '<span style="background:#fefce8; color:#a16207; padding:4px 10px; border-radius:6px; font-size:0.75rem; font-weight:800; letter-spacing:0.02em;">≡ƒƒí WARM BUYER</span>';
            ...
            <article class="card glass" style="padding:24px; border-left:6px solid ${accent}; border-radius:16px;">
                ...
                ${isHot ? `<div style="display:inline-block; background:#eff6ff; color:#1d4ed8; font-size:0.8rem; font-weight:800; padding:6px 12px; border-radius:8px; margin-bottom:12px;"><i class="fas fa-lock"></i> Assigned exclusively to YOU for 2 hours</div>` 
                       : `<div style="display:inline-block; background:#f3f4f6; color:#4b5563; font-size:0.8rem; font-weight:800; padding:6px 12px; border-radius:8px; margin-bottom:12px;">≡ƒæñ Assigned to YOU</div>`}
                ...
                <div style="background:rgba(255,255,255,0.7); border:1px solid var(--line); border-radius:12px; padding:16px; margin-bottom:20px;">
                    ...
                    <p style="margin:8px 0; font-size:0.95rem; font-family:'Plus Jakarta Sans'; font-weight:600; color:var(--ink);">"Hi ${esc(signal.userName)}, I saw you're looking at ${propTitle} ΓÇö I have 2 off-market units that match exactly what you want. Want me to send them?"</p>
