...
// Keep only lines 0 to 3450
const cleanLines = lines.slice(0, 3451);
...
const aiLogic = [
    ...
    '        if(lower.includes("invest") || lower.includes("roi")) {',
    '            const p = (typeof properties !== "undefined" && properties[0]) || {};',
    '            response = "<strong>Investment Analysis</strong><br>" +',
    '                "ROI: " + ((p.growth || 0) + (p.yield || 0)) + "%<br>" +',
    '                "Rental Yield: " + (p.yield || 0) + "%<br>" +',
    '                "Capital Growth: " + (p.growth || 0) + "% YOY<br><br>" +',
    '                "<button class=\\"btn\\" style=\\"width:100%;margin-top:8px;\\" onclick=\\"contactAgent()\\">Connect me to an agent now!</button>";',
    '        } else if(lower.includes("afford") || lower.includes("budget")) {',
    '            response = "<strong>Budget Check!</strong><br>For a RM500k home, you generally need a combined net income around RM4,500/month. Want me to open the calculator?";',
    '        } else if(lower.includes("area") || lower.includes("good")) {',
    '            response = "<strong>Great question!</strong><br>Areas like Mont Kiara and Bukit Jalil are growing at about 6.5% a year. Do you have a favorite spot in mind?";',
    '        } else if(lower.includes("compare")) {',
    '            var saved = (typeof savedIds !== "undefined") ? savedIds.length : 0;',
    '            if(saved > 0) {',
    '                response = "<strong>Let\'s compare!</strong><br>You\'ve saved " + saved + " homes! Most have a yield profile over 4.5%.";',
    '            } else {',
    '                response = "You haven\'t saved any homes yet! Tap the heart icon on properties you love.";',
    '            }',
    '        } else {',
    '            if(typeof chatbotReply === "function") {',
    '                response = "<strong>Here\'s what I found!</strong><br>" + chatbotReply(text);',
    '            } else {',
    '                response = "I can help you check prices, growth, or area vibes instantly. What are you wondering about?";',
    '            }',
    '        }',
    ...
    '            };',
    '            writeLocalChats(readLocalChats().concat([entry]));',
...
const aiHtml = [
    ...
    '            I\'m your friendly Property Guide! I can help you with affordability, neighborhoods, investment ROI, and more. How can I help today?',
    '        </div>',
    '    </div>',
    '    <div class="chat-prompts">',
    '        <button class="chat-prompt-chip" onclick="quickSendChat(\'Can I afford this?\')">Can I afford this?</button>',
    '        <button class="chat-prompt-chip" onclick="quickSendChat(\'Is this a good investment?\')">Is this a good investment?</button>',
...
if (headEnd !== -1) {
    result = result.slice(0, headEnd) + aiCss + '\n' + result.slice(headEnd);
}

fs.writeFileSync('dashboard.html', result, 'utf8');
console.log('Fixed duplication entirely. Size is now: ' + result.length);
